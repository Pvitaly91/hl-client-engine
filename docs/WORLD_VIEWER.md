...
slow deterministic bounds orbit. There are no keyboard/mouse gameplay controls
or spawn-camera mode. A positive `HLCLIENT_SMOKE_TEST_FRAMES` value bounds the
frame loop for smoke/automation runs; normal manual use runs until quit.
...
The M4.3 preview shows static model 0 geometry with resolved embedded and WAD3
base textures, masked index-255 surfaces, the baseline first light-style layer,
...
The preview is not expected to show a correct player spawn viewpoint, dynamic
light-style animation, dynamic lights, doors or other brush entities, a
skybox, animated water, PVS/frustum traversal, gameplay movement, or live
server state.
...
one draw call and a nonzero triangle count, rejects created/deleted/modified
files, and prints metadata only. `maps/boot_camp.bsp`, `maps/crossfire.bsp`,
...
network/resource pipeline, builds the same immutable package, finalizes the
retained netchan and authentication lifetime exactly once, and only then opens
the local OpenGL preview. It requires `--renderer opengl` and honors the same
frame-limit environment variable. The resulting view is a diagnostic snapshot
of resources received before cleanup, not a gameplay-connected session.

For a CPU-only proof, `--stop-after world-render-package` builds and validates
the package with no SDL window or GPU upload and is valid with
`--renderer null`.
