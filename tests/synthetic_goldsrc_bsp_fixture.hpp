...
// Independent literal collision BSP v30 fixture. Unlike the test-only builder
// below, this complete byte string itself contains the plane, hull-0 node and
// leaves, one eight-byte clipnode, and one model whose four headnodes all use
// source index zero. The node and clipnode terminals preserve exact solid (-2)
// and empty (-1) contents. Keep this literal independent so parser/package
// expectations cannot accidentally track the writer implementation.
inline constexpr char kLiteralCollisionGoldSrcBspV30[] =
    "\x1e\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x7c\x00\x00\x00"
    "\x14\x00\x00\x00\x90\x00\x00\x00\x30\x00\x00\x00\xc0\x00\x00\x00"
    "\x30\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xf0\x00\x00\x00"
    "\x18\x00\x00\x00\x08\x01\x00\x00\x28\x00\x00\x00\x30\x01\x00\x00"
    "\x14\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x44\x01\x00\x00"
    "\x08\x00\x00\x00\x4c\x01\x00\x00\x38\x00\x00\x00\x84\x01\x00\x00"
    "\x02\x00\x00\x00\x86\x01\x00\x00\x14\x00\x00\x00\x9a\x01\x00\x00"
    "\x10\x00\x00\x00\xaa\x01\x00\x00\x40\x00\x00\x00\x00\x00\x00\x00"
    "\x00\x00\x00\x00\x00\x00\x80\x3f\x00\x00\x00\x00\x02\x00\x00\x00"
    "\x01\x00\x00\x00\x08\x00\x00\x00\x54\x45\x53\x54\x5f\x51\x55\x41"
    "\x44\x00\x00\x00\x00\x00\x00\x00\x40\x00\x00\x00\x40\x00\x00\x00"
    "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    ...
    "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    "\x00\x00\x00\x00\x00\x00\x00\x00\x04\x00\x00\x00\xff\xff\xff\xff"
    "\xff\xff\xff\xff\x00\x00\x00\x00\xff\xff\xfe\xff\xfe\xff\xff\xff"
    "\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\x41\x00\x41\x00\x01\x00"
    "\x00\x00\x00\x00\x00\x00\x00\x00\xff\xff\xff\xff\xff\xff\xff\xff"
    "\xff\xff\xff\xff\xff\xff\x41\x00\x41\x00\x01\x00\x00\x00\x01\x00"
    "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00\x01\x00"
    "\x02\x00\x02\x00\x03\x00\x03\x00\x00\x00\xfc\xff\xff\xff\xfd\xff"
    "\xff\xff\xfe\xff\xff\xff\xff\xff\xff\xff\x00\x00\x80\xbf\x00\x00"
    "\x80\xbf\x00\x00\x80\xbf\x00\x00\x82\x42\x00\x00\x82\x42\x00\x00"
    "\x80\x3f\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
    "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x00"
    "\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00";

static_assert(sizeof(kLiteralCollisionGoldSrcBspV30) - 1U == 490U);

[[nodiscard]] inline std::vector<std::byte> literal_collision_goldsrc_bsp_v30()
{
    std::vector<std::byte> bytes;
    bytes.reserve(sizeof(kLiteralCollisionGoldSrcBspV30) - 1U);
    for (std::size_t index = 0U;
         index < sizeof(kLiteralCollisionGoldSrcBspV30) - 1U;
         ++index) {
        bytes.push_back(static_cast<std::byte>(
            static_cast<unsigned char>(kLiteralCollisionGoldSrcBspV30[index])));
    }
    return bytes;
}
